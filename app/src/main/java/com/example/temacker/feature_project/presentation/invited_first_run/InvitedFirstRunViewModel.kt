...
class InvitedFirstRunViewModel(
    ...
    init {
        val project = observeUserProjects()
            .mapNotNull { result -> (result as? Result.Success)?.data?.firstOrNull() }
            .distinctUntilChanged()
        val projectId = project.mapNotNull { it.id }.distinctUntilChanged()

        viewModelScope.launch {
            project.collect { p -> _state.update { it.copy(projectName = p.name) } }
        }
        viewModelScope.launch {
